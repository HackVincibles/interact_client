...
const Agent = ({
    ...
    const attachStream = () => {
      if (stream) {
        ...
        if (coachVideoRef.current) {
          ...
          coachVideoRef.current.play().catch((err) => console.warn("[AI Coach] coach video play failed:", err));
        }
      }
    };
  ...
  return (
    <div className="flex flex-col w-full h-screen overflow-hidden">
      ...
      <video ref={coachVideoRef} autoPlay playsInline muted className="absolute pointer-events-none opacity-0 w-1 h-1" />
